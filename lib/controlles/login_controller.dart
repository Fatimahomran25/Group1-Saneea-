...
if (email_admin == null) {
  ...
  Future<String?> loginAndGetRole() async {
  serverError = null;
  submitted = true;
...
/*if (nid == '1120304050') {
  ...
  try {
    // نجيب الإيميل من admins أو users
    String? email = await _emailByAdminId(nid);
    final isAdmin = email != null;

    email ??= await _emailByNationalId(nid);
    ...
    return accountType.isEmpty ? 'client' : accountType;
  ...
  try {
    // 1) حاول نجيب الإيميل من admins أولاً
    String? email = await _emailByAdminId(nid);

    // 2) إذا مو admin، جيبه من users
    email ??= await _emailByNationalId(nid);

    // 3) إذا ما لقينا أي إيميل
    if (email == null) {
      serverError = 'No account found for this ID.';
      return false;
    }

    // 4) إرسال رابط تغيير كلمة المرور
